describe('Coral.Alert', function() {
  ...
  describe('API', function() {
    ...
    describe('#size', function() {
      it('should default to small', function(done) {
        ...
        // helpers.next(function() {
        expect(el.attributes.getNamedItem('size').value).to.equal(Coral.Alert.size.SMALL);
        done();
        // });
      });
  ...
  describe('Tracking', function() {
    ...
    it('should call the tracker callback fn with at least four parameters: trackData, event, component, childComponent when the alert is closed', function(done) {
      helpers.build(window.__html__['Coral.Alert.tracking.html'], function(el) {
        ...
        expect(trackData).to.have.property('targetType', 'alert');
        ...
        expect(trackData).to.have.property('rootType', 'alert');
        expect(spyCall.args[1]).to.be.an.instanceof(MouseEvent);
        expect(spyCall.args[2]).to.be.an.instanceof(Coral.Alert);
        done();
      });
