    ...
    scrollToSection : function(id){
        ...
        if(el){
            ...
            this.body.scrollTo('top', top-25, {duration:.5, callback: function(){
                Ext.fly(el).next('h2').pause(.2).highlight('#cccccc', {attr:'color'});
            }});
        }
    },
...
//            url: 'http://extjs.com/playpen/api.php'
    ...
    MainPanel.superclass.constructor.call(this, {
        ...
        items: {
            id:'welcome-panel',
            title: 'API Home',
            autoLoad: {url: 'resources/welcome.html', callback: this.initSearch, scope: this},
            iconCls:'icon-docs',
            autoScroll: true,
            tbar: [
                'Search: ', ' ',
                new Ext.ux.SelectBox({
                    ...
                    store: new Ext.data.SimpleStore({
                        fields: ['text'],
                        expandData: true,
                        data : ['Starts with', 'Ends with', 'Any match', 'Exact match']
                    }),
                    displayField: 'text'
                }), ' ',
...
Ext.onReady(function(){
    ...
    api.on('click', function(node, e){
         if(node.isLeaf()){
            e.stopEvent();
            mainPanel.loadClass(node.attributes.href+"?internal=1", node.id);
         }
    });
...
Ext.app.SearchField = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    addRecords: function(queue) {
		...
		// console.log("working queue for ",queue[0].cls);
		while (queue.length > 0) {
			var match = queue.shift();
			var cls = match.cls;
			var member = match.token[0];
	        var type = this.searchInfo[1][match.token[1]];
			var doc = "";
			var clsInfo = this.searchInfo[3][cls];
			if (clsInfo[type + "s"]) {
				var members = clsInfo[type + "s"];
	        ...
	        this.store.addSorted(new Ext.data.Record(rec));
		}
    },
    ...
    search: function(query, queryType) {
		...
		this.queue = {};
        var sf = this;
        for (var i = 0; i < this.searchInfo[0].length; i++) {
            var token = this.searchInfo[0][i];
            var fullName = token[0];
            var shortName = fullName.substring(fullName.lastIndexOf(".") + 1);
            var name = shortName.toLowerCase();
            var match;
            ...
            switch (queryType) {
                ...
                case "Ends with":
                    match = (name.indexOf(query) >= 0 && 
                             name.indexOf(query) == name.length - query.length);
                    break;
                case "Starts with":
                default:
                    match = (name.indexOf(query) == 0);
            ...
            if (match) {
				var cls = this.searchInfo[2][token[2]];
				// add match to queue for cls
				var noQueue = !this.queue[cls] || this.queue[cls].length == 0;
				if (noQueue) {
					this.queue[cls] = [];
				}
				this.queue[cls].push({
 				...
 				if (!this.searchInfo[3][cls]) {
					if (noQueue) {
                    // retrieve cls info
	                    var clsInfo = new Ext.data.Connection({
	                        url:"output/" + cls + ".json",
	                    ...
	                    clsInfo.request({
	                        ...
	                        success:function(r, o) {
								var ocls = o.cls;
	                            sf.searchInfo[3][ocls] = eval('(' + r.responseText + ')');
								sf.addRecords(sf.queue[ocls]);
	                        }
	                    });
	                }
                } else {
                    // cls info cached
                    this.addRecords(this.queue[cls]);
                }
            }
        }
    },
    ...
    onTrigger2Click : function(){
        ...
        this.store.fireEvent("beforeload", this.store); // fake store loading
        var sf = this;
        // init search
        if (this.isInited == undefined) {
            ...
            window.setTimeout(function() {
                var index = new Ext.data.Connection({
                    url:"search.json",
                    disableCaching:false
                });
                index.request({
                    success:function(r, o) {
                        sf.searchInfo = eval(r.responseText);
                        sf.searchInfo.push({}); // cls info cache
                        sf.isInited = true;
                        sf.onTrigger2Click();
                    }
                });
            ...
            return;
        } else if (this.isInited === false) {
            // still initializing search
            return;
        }
        var qt = Ext.getCmp('search-type').getValue();
        ...
        window.setTimeout(function() {
            if (sf.clearStore) {
                var emptyQueue = true;
                for (var cls in sf.queue) {
                    emptyQueue = false;
                    break;
                }
                if (emptyQueue) {
                    // there is no queue, abort loading
                    sf.store.removeAll();
                    sf.clearStore = false;
                }
            }
