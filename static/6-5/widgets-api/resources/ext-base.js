...
(function() {
    ...
    Ext.apply(Ext, {isStrict:isStrict,isSecure:isSecure,isReady:false,enableGarbageCollector:true,enableListenerCollection:false,SSL_SECURE_URL:"javascript:false",BLANK_IMAGE_URL:"http://extjs.com/s.gif",emptyFn:function() {
        ...
        if (t == "object" || t == "function") {
            switch (o.constructor) {case Array:return"array";case RegExp:return"regexp";case Date:return"date"}
...
(function() {
                ...
                if (u && u.length > 0) {
                    for (var r = 0,t = u.length; r < t; ++r) {
                        ...
                        if (o && o[this.EL] === n && (!s || s === o[this.TYPE])) {
                            q.push({type:o[this.TYPE],fn:o[this.FN],obj:o[this.OBJ],adjust:o[this.ADJ_SCOPE],index:r})
                        }
                    }
                }
            }
    ...
    d._tryPreloadAttach();
    Ext.lib.Ajax = {request:function(l, j, e, k, f) {
        if (f) {
            var g = f.headers;
            if (g) {
                for (var i in g) {
        ...
        for (var n = 0; n < f.elements.length; n++) {
            ...
            if (!l && e) {
                switch (g.type) {case"select-one":case"select-multiple":for (var k = 0; k < g.options.length; k++) {
                ...
                }break;case"file":case undefined:case"reset":case"button":break;case"submit":if (o == false) {
                    m += encodeURIComponent(e) + "=" + encodeURIComponent(h) + "&";
                    o = true
                }break;default:m += encodeURIComponent(e) + "=" + encodeURIComponent(h) + "&";break}
            }
        }
        ...
        if (h == 0 ||(h >= 200 && h < 300) || (Ext.isIE && h == 1223)) {
            ...
            switch (h) {case 12002:case 12029:case 12030:case 12031:case 12152:case 13030:g = this.createExceptionObject(j.tId, k.argument, (f ? f : false));if (k.failure) {
                if (!k.scope) {
                    k.failure(g)
                } else {
                    k.failure.apply(k.scope, [g])
                }
            }break;default:g = this.createResponseObject(j, k.argument);if (k.failure) {
                if (!k.scope) {
                    k.failure(g)
                } else {
                    k.failure.apply(k.scope, [g])
