  ...
  describe('attributes', function() {
    describe('#sticky', function() {
      ...
      it('should not set min-width on header cell without visible content', function(done) {
        var table = helpers.build(window.__html__['Coral.Table.sticky.empty.headercell.html'], function() {
          table._wait = 0;
          // Because of debouncing
          window.setTimeout(function() {
            var headerCells = table._elements.head._elements.rows[0]._getCells(['headerCells']);
      ...
      it('should handle long header cell label with short content without overflowing', function(done) {
        var table = helpers.build(window.__html__['Coral.Table.sticky.labeloverlap.html'], function() {
          table._wait = 0;
          // Because of debouncing
          window.setTimeout(function() {
            var longLabelHeaderCell = table._elements.head._elements.rows[0]._getCells(['headerCells'])[3];
            var labelWidth = longLabelHeaderCell.clientWidth;
            
            table._elements.head.sticky = false;
