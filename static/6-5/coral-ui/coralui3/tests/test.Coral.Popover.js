describe('Coral.Popover', function() {
  ...
  describe('Implementation details', function() {
    ...
    it('should focus the focusOnShow element when opened', function(done) {
      helpers.build(window.__html__['Coral.Popover.focusOnShow.html'], function(el) {
  ...
  describe('Tracking', function() {
    ...
    it('should not call the tracker callback fn when the popover is initialized and not opened', function(done) {
      helpers.build(window.__html__['Coral.Popover.tracking.html'], function() {
        expect(trackerFnSpy.callCount).to.equal(0, 'Tracker was called.');
        done();
      });
    });

    it('should call the tracker callback fn with expected parameters when the popover is manually opened', function(done) {
      helpers.build(window.__html__['Coral.Popover.tracking.html'], function(el) {
        ...
        Coral.commons.ready(popover, function() {
          ...
          expect(trackData).to.have.property('targetType', 'popover');
          ...
          expect(trackData).to.have.property('rootType', 'popover');
          expect(spyCall.args[1]).to.be.an.instanceof(MouseEvent);
          expect(spyCall.args[2]).to.be.an.instanceof(Coral.Popover);
          done();
        });
      });
    });

    it('should call the tracker callback fn with expected parameters when the popover is closed by clicking on the body', function(done) {
      helpers.build(window.__html__['Coral.Popover.tracking.autoOpen.html'], function(el) {
    ...
    it('should call the tracker callback fn with expected parameters when the popover is closed by clicking on the "X" closable button', function(done) {
      helpers.build(window.__html__['Coral.Popover.tracking.autoOpen.html'], function(el) {

        var popover = el.querySelector('coral-popover');

        Coral.commons.ready(popover, function() {
          popover.querySelector('button[variant="minimal"]').click();
