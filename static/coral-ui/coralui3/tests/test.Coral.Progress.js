describe('Coral.Progress', function() {
  ...
  it('should have the right className', function() {
    ...
    expect(progress.classList.contains('coral3-Progress')).to.be.true;
  });
  ...
  describe('Public API', function() {
    ...
    it('label should be hidden when showPercent is off and label has no text content', function() {
      ...
      expect(progress.classList.contains(progress._className + '--noLabel')).to.be.true;
    });
    ...
    it('should set the correct classname when switching sizes', function(done) {
      ...
      helpers.next(function() {
        expect(progress.classList.contains(progress._className+'--large')).to.be.true;
        expect(progress.classList.contains(progress._className+'--small')).to.be.false;
        expect(progress.classList.contains(progress._className+'--medium')).to.be.false;

        progress.size = Coral.Progress.size.SMALL;
        helpers.next(function() {
          expect(progress.classList.contains(progress._className+'--small')).to.be.true;
          expect(progress.classList.contains(progress._className+'--medium')).to.be.false;
    ...
    it('should set the correct className when switching label positions', function(done) {
      ...
      helpers.next(function() {
        expect(progress.classList.contains(progress._className+'--rightLabel')).to.be.true;
        expect(progress.classList.contains(progress._className+'--leftLabel')).to.be.false;
        expect(progress.classList.contains(progress._className+'--bottomLabel')).to.be.false;

        progress.labelPosition = 'bottom';

        helpers.next(function() {
          expect(progress.classList.contains(progress._className+'--bottomLabel')).to.be.true;
          expect(progress.classList.contains(progress._className+'--leftLabel')).to.be.false;
