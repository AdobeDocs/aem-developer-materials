...
describe('CUI.Autocomplete', function () {
  ...
  describe('selectlist visibility', function () {
    ...
    it('shows when removing text that had matching options', function () {
      var clock = sinon.useFakeTimers();
      $input.val('Bah').trigger('input');
      clock.tick(100000); // Skip the debounce wait.
    ...
    it('shows when cutting matching text from input', function () {
      // Fast forward past any potential debounce.
      // We want to make sure it doesn't show up again.
      var clock = sinon.useFakeTimers();
  ...
  describe('Options API', function() {
    ...
    describe('disabled', function() {
      ...
      beforeEach(function() {
        widget = $autocomplete.data('autocomplete');
      });

      it('should be false by default', function() {
        expect(widget.get('disabled')).to.be.false;
        expect(widget.$element.hasClass('is-disabled')).to.be.false;
        expect(widget._input.prop('disabled')).to.be.false;
        expect(widget._suggestionsBtn.prop('disabled')).to.be.false;
      });
      ...
      it('should be possible to set at runtime', function() {
        ...
        expect(widget.$element.hasClass('is-disabled')).to.be.true;
        expect(widget._input.prop('disabled')).to.be.true;
        expect(widget._suggestionsBtn.prop('disabled')).to.be.true;
      });
    });
  });
