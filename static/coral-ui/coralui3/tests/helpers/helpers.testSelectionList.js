helpers.testSelectionList = function(Group, GroupItem) {
  ...
  var VALID_ARIA_SELECTED_ROLES_REGEXP = new RegExp('^('+ VALID_ARIA_SELECTED_ROLES.join('|') +')$');
      ...
      it('should take the last selected if not multiple, post init', function(done) {
        ...
        helpers.next(function() {
          ...
          helpers.next(function() {
            ...
            if (VALID_ARIA_SELECTED_ROLES_REGEXP.test(items[2].getAttribute('role'))) {
                expect(items[2].getAttribute('aria-selected')).to.equal('true');
            } else {
                expect(items[2].hasAttribute('aria-selected')).to.be.false;
            }

            expect(items[0].hasAttribute('selected')).to.be.false;

            if (VALID_ARIA_SELECTED_ROLES_REGEXP.test(items[0].getAttribute('role'))) {
                expect(items[0].getAttribute('aria-selected')).to.equal('false');
            } else {
                expect(items[0].hasAttribute('aria-selected')).to.be.false;
            }
