...
describe('Coral.StepList', function() {
  ...
  function testDefaultInstance(el) {
    ...
    expect(el.getAttribute('selectedItem')).to.be.falsy;
    expect(el.getAttribute('variant')).to.be.falsy;
    expect(el.getAttribute('items')).to.be.falsy;
  }
  ...
  describe('Markup', function() {
    ...
    describe('#selectedItem', function() {
      ...
      it('should take the last selected', function(done) {
        helpers.build(window.__html__['Coral.StepList.doubleselected.html'], function(el) {
          ...
          helpers.next(function() {
            ...
            expect(items[1].getAttribute('selected')).to.be.falsy;
            expect(items[2].selected).to.be.true;
            done();
          });
        });
      ...
      it('should switch for one long label the other labels to hidden', function(done) {
        helpers.build(window.__html__['Coral.StepList.longlabel.html'], function(el) {
          var items = el.items.getAll();
      ...
      it('should not center the one long label when switching back from hybrid to normal mode', function(done) {
        helpers.build(window.__html__['Coral.StepList.longlabel.html'], function(el) {
  ...
  describe('Implementation Details', function() {
    ...
    it('should select the target after being inserted in the DOM', function(done) {
      helpers.build(window.__html__['Coral.StepList.virtualtarget.html'], function(el) {
        var clone = document.importNode(el.content, true);
        helpers.target.appendChild(clone);

        helpers.next(function() {
          var tabItem = document.getElementById('stepItem');
  ...
  describe('Tracking', function() {
    ...
    it('should not call the tracker callback if interaction is OFF', function(done) {
      helpers.build(window.__html__['Coral.StepList.tracking.html'], function(el) {
        // Turn interaction on
    ...
    it('should call tracker callback with the expected tracker data when first step is clicked', function(done) {
      helpers.build(window.__html__['Coral.StepList.tracking.html'], function(el) {
        ...
        expect(trackData).to.have.property('targetType', 'steplist-item');
        ...
        expect(trackData).to.have.property('rootType', 'steplist');
        done();

      });
    });

    it('should call tracker callback with the expected tracker data when an annotated step is clicked', function(done) {
      helpers.build(window.__html__['Coral.StepList.tracking.html'], function(el) {
