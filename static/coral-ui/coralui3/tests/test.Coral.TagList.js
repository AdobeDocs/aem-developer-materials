...
describe('Coral.TagList', function() {
  ...
  describe('Instantiation', function() {
    ...
    it('should be possible to clone using js', function(done) {
      ...
      helpers.next(function() {
        ...
        // Needs additional frame to make sure the tagList was added to the DOM
        helpers.next(function() {
  ...
  describe('API', function() {
      ...
      it('should create an input related to the tag item', function(done) {
        var tagList = helpers.build(window.__html__['Coral.TagList.base.html'], function() {
          tagList.items.getAll().forEach(function(item) {
            expect(item.querySelector('input[type="hidden"]')).to.equal(item._input);
          });
          done();
        });
      });
    ...
    it('should reset the taglist', function(done) {
      var tagList = helpers.build(window.__html__['Coral.TagList.base.html'], function() {
        tagList.reset();
        helpers.next(function() {
          var item = tagList.items.first();
          expect(item._input).to.not.equal(undefined);
          expect(item.closable).to.be.true;
  ...
  describe('Tracking', function() {
    ...
    it('should call the tracker callback fn with expected parameters when the a tag is removed', function(done) {
      helpers.build(window.__html__['Coral.TagList.tracking.full.html'], function(el) {
        ...
        Coral.commons.ready(el, function() {
          ...
          expect(trackData).to.have.property('targetType', 'taglist-tag');
          ...
          expect(trackData).to.have.property('rootType', 'taglist');
          expect(spyCall.args[1]).to.be.an.instanceof(CustomEvent);
          expect(spyCall.args[2]).to.be.an.instanceof(Coral.TagList);
          done();
        });
