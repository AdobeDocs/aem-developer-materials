describe('Coral.mixin.selectionList', function() {
  ...
  var SNIPPET_ARIA_SELECTED = window.__html__['Coral.mixin.selection.selectionList.aria-selected.html'];
...
Coral.register({
    tagName: 'coral-selectionlist-aria-selected-example',
    name: 'SelectionListAriaSelectedExample',
    mixins: [
      Coral.mixin.selectionList({
        itemTagName: 'coral-selectionlist-aria-selected-exampleitem',
        role: 'tablist'
      })
    ]
  });
  ...
  Coral.register({
    tagName: 'coral-selectionlist-aria-selected-exampleitem',
    name: 'SelectionListAriaSelectedItemExample',
    mixins: [
      Coral.mixin.selectionList.Item({
        listSelector: 'coral-selectionlist-aria-selected-example',
        role: 'tab'
      })
    ]
  });
  ...
  describe('API', function() {
    ...
    it('should add an item with the right role', function(done) {
      ...
      buildTwoChildrenSnippet(function(selectionList) {
        ...
        helpers.next(function() {
          ...
          expect(eventSpy.args[0][0].detail.item.getAttribute('role')).to.equal('coral-item-role');
          ...
          for (var i = 0; i < selectionList.childElementCount; i++) {
            expect(children[i].getAttribute('role')).equal('coral-item-role');
          }
          selectionList.off('coral-collection:add', eventSpy);
          done();
        });
      });
    });
    ...
    it('should remove an item', function(done) {
      ...
      buildTwoChildrenSnippet(function(selectionList) {
        ...
        helpers.next(function() {
          ...
          selectionList.off('coral-collection:remove', eventSpy);
          done();
        });
      });
    });
    ...
    describe('single mode', function() {
      it('should remove double set selection at init time', function(done) {
        helpers.build(SNIPPET_DOUBLE_PRESET_SINGLE, function(list) {
          ...
          expect(list.querySelector('[selected]')).to.not.equal(null);
          done();
        });
      });
    });
    ...
    describe('(private) _getSelectionList', function() {
      ...
      it('should return the correct parent', function() {
        ...
        expect(item._getSelectionList(), 'should be undefined').to.equal(null);
    ...
    describe('(private) _oldSelection', function() {
      it('should match the first item', function(done) {
        ...
        helpers.build(window.__html__['Coral.mixin.selection.selectionList.base.html'], function(el) {
          ...
          document.removeEventListener('coral-selectionlist-example:change', changeSpy);
    ...
    describe('Accessibility', function() {
      it('should add aria-selected when item has appropriate role', function(done) {

        helpers.build(SNIPPET_ARIA_SELECTED, function(list) {
          expect(list.getAttribute('role')).to.equal('tablist');
          var items = list.items.getAll();
          expect(items[0].getAttribute('role')).to.equal('tab');
          expect(items[1].getAttribute('role')).to.equal('tab');
