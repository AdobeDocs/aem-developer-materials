describe('CUI.Select', function() {
  ...
  describe('Initialization/Setup', function() {
    ...
    it('should mark selectList item selected for single select preselected markup', function() {
      var widget = $(singleSelectPreselectedMarkup).select().data('select');
      var $selectListOptionSelected = widget.$element.find('.coral-SelectList-item--option[aria-selected="true"]');

      // Should set aria-selected=true for selected list item
      expect($selectListOptionSelected.data('value')).to.equal(2);
    });
  ...
  describe('Options API', function() {
    // Related to changes in option values.
    ...
    describe('disabled', function() {
      ...
      it('should be false by default', function() {
        expect(widget.get('disabled')).to.be.false;
        expect(widget._button.hasClass('is-disabled')).to.be.false;
        expect(widget._button.prop('disabled')).to.be.false;
        expect(widget._nativeSelect.prop('disabled')).to.be.false;
      });

      it('should be possible to set at runtime', function() {
        widget.set('disabled', true);

        expect(widget.get('disabled')).to.be.true;
        expect(widget._button.hasClass('is-disabled')).to.be.true;
        expect(widget._button.prop('disabled')).to.be.true;
        expect(widget._nativeSelect.prop('disabled')).to.be.true;
      });
    });
  });
